...
def slerp_batched(alpha: float, point_a: torch.Tensor, point_b: torch.Tensor) -> torch.Tensor:
    ...
    theta = torch.arccos(
        torch.sum(point_a_vec * point_b_vec, dim=1)
        / (torch.norm(point_a_vec, dim=1) * torch.norm(point_b_vec, dim=1))
    )
    interp = (
        (torch.sin((1 - alpha) * theta) / torch.sin(theta)) * point_a_vec
        + (torch.sin(alpha * theta) / torch.sin(theta)) * point_b_vec
    )
